...
# # ROC CURVES
# preds_prob <- read.table("preds_prob.txt")
# authors <-  y.test %>% as_tibble()
# preds_prob <- cbind(authors, preds_prob) # add column for authors 
# preds_prob["AaronPressman"] %>% head()
# 
# 
# 
# 
# authname <- levels(y.test)[2]
# preds_prob %>% filter_if(is.double, any_vars(.==1)  )
# 
# 
# 
# confmat_by_author <- preds_prob %>% 
#   as_tibble() %>%
#   filter(value == authname) %>% 
#   select(value, authname, everything() ) %>%               # reorder so its easier to see
#   mutate_if(is.double, list(~ ifelse(. > 0.1, 1, 0) )) 
#   
# 
# confmat_by_author[confmat_by_author == 1]
# 
# which(confmat_by_author==1)
# confmat_by_author[ , confmat_by_author==1 ]
# rowSums(confmat_by_author[-1])
# 
# 
# # all values over threshold = 1, otherwise 0
# 
# TP = sum(confmat_by_author[,2])
# sum(confmat_by_author[,-1:-2])
# metric_on_confmat(confmat_by_author[,-1] %>% as.matrix())
# m <- confmat_by_author[,-1] %>% as.matrix()
# all_predicted_positives <- sum(m)
# all_predicted_positives
# TP
# FP = all_predicted_positives - TP
# m[m==0]
# 30/50
# 
# ?confusionMatrix
# confusionMatrix(ifelse(preds_prob > 0.1, 1, 0), y.test)
# colnames(preds_prob) <- levels(y.test)
# 
# preds_prob[1:10,1:10]
# thresholds <- seq(0, 1, 0.01)
# n_classes <- ncol(preds_prob)
# 
# 
# ifelse (preds_prob[50:55,] > 0.1 , 1, 0)
# y.test[[1]]
# authors <- y.test %>% as_tibble()
# 
# tt <- ifelse (preds_prob[1:4,1:4] > thresholds[19] , 1, 0) %>% as.table()
# str(tt)
# nn <- rowSums(tt) %>% as.numeric()
# dd <- diag(tt) %>% as.numeric()
# tpr <- dd/nn
# tpr[which(is.nan(tpr) )] <- 0
# tpr
# tpr[is.nan(tpr)] = 0
# which(is.nan(tpr) )
# tpr
# 
# for (t in thresholds){
#   confmatrix <- ifelse (preds_prob > thresholds[t] , 1, 0)
#   
#   
